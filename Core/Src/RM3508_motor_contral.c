...
struct rx_date_motor_rm3508_struct motor_rx_data[4]; // Array to hold data for 4 motors
...
void RM3508_PID_Motor_Init(void)
{
    ...
    xTaskCreate(RM3508_Motor_get, "RM3508_Motor_get", 256, NULL, 1, NULL);
    speed_queue = xQueueCreate(1, sizeof(float)); // Create a queue for speed
    angle_queue = xQueueCreate(1, sizeof(float)); // Create a queue for angle
}
void RM3508_Motor_get(void *argument)
{
    while (1)
    {
        struct rx_date_motor_rm3508_struct motor_rx_data_temp;
        xQueueReceive(motor_rm3508_rx_queue, &motor_rx_data_temp, portMAX_DELAY); // Peek the motor data from the queue
        switch (motor_rx_data_temp.motor_number)
        {
        case 0:
            motor_rx_data[0] = motor_rx_data_temp;
            break;
        case 1:
            motor_rx_data[1] = motor_rx_data_temp;
            break;
        case 2:
            motor_rx_data[2] = motor_rx_data_temp;
            break;
        case 3:
            motor_rx_data[3] = motor_rx_data_temp;
            break;
        }
    }
}
...
void RM3508MotorSetSpeed(void *argument)
{
    ...
    {
        float sp;
        xQueuePeek(speed_queue, &sp, 0); // Peek the current speed from the queue
        float co;
        float fb = 0;
